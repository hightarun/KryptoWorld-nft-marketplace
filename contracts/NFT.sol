...
contract NFT is ERC721URIStorage {
    ...
    // setApporvalForAll allows us to do that with contract address

    // constructor set up our address
    constructor(address marketplaceAddress) ERC721("KryptoWorld", "KWORLD") {
    ...
    function mintToken(string memory tokenURI) public returns (uint256) {
        ...
        // passing id and url for minting
        _mint(msg.sender, newItemId);

        // set the token URI: id and url
        _setTokenURI(newItemId, tokenURI);

        //give the MP the approval to transact b/w users
        setApprovalForAll(contractAddress, true);

        //mint the token and set it for sell - return the id to do so
        return newItemId;
    }
}
